...
class Matcher:
    """
    Matcher Class -- Match data for an observational study. Build.
    ...
    def __init__(self, test, control, yvar, formula=None, exclude=[]):
        ...
        c.index += len(t)
        self.data = pd.concat([t.dropna(axis=1, how='all'), c.dropna(axis=1, how='all')], sort=True)
    ...
    def balanced_sample(self, data=None):
        ...
        return pd.concat([major.sample(len(minor)),minor], sort=True).dropna()
